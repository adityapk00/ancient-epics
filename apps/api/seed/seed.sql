...
INSERT INTO app_settings (key, value) VALUES
    ...
    ('admin_ingestion_prompt', 'You are preparing a bilingual reading edition for Ancient Epics. Split the source chapter into paired bilingual chunks that preserve all source content. Return JSON only with this shape: {"chapterTitle": string, "notes": string, "chunks": [{"originalText": string, "translatedText": string, "type": "verse" | "prose"}]}. The concatenation of all originalText values must exactly reproduce the source chapter without dropping or duplicating content.');
